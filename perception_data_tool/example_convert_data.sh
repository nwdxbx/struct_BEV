...
source_dir="/media/zhengyim/hard/xubenxiang/data"

BAG_DIR="/media/zhengyim/hard/xubenxiang/srcbags"
# 点云话题
# TOPIC_NAME=$2
...
# 遍历指定目录下的所有文件和子目录,提取gnss文件信息
gnss_topics=(
    '/localization/gnss' 
    )
gnss_topics_str="${gnss_topics[@]}"
for FILE in $(find "$BAG_DIR" -type f); do
    echo "File: $FILE"
        PACKAGE_NAME=$(basename "$FILE" .bag)
        FINAL_LIDAR_DIR="$OUTPUT_DATA_DIR/$PACKAGE_NAME/"
        mkdir -p $FINAL_LIDAR_DIR &
        echo "package_name: $PACKAGE_NAME"
        # 提取点云
        python3 parse_localization.py --out_dir $FINAL_LIDAR_DIR \
        --input_bag  $FILE --topics  $gnss_topics_str
        sleep 5
        echo "gnss conversion done."
done
